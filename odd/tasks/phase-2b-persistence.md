...
- 2026-09-23: T3 done on `feat/2b-t3-client-wiring`. Added pure, tested `src/features/practice/decisionRequest.ts` (`buildDecisionRequest` — URL = `${NEXT_PUBLIC_BASE_PATH ?? ""}/api/decisions`, JSON body carrying only `playerCards`/`dealerUpcard`/`userAction`, never the client's own grading fields) and `src/features/practice/sendDecision.ts` (fire-and-forget `fetch` with `keepalive: true`, `credentials: "same-origin"`; wraps both the call and its `.catch` so a synchronously-throwing or rejecting `fetch` can never throw out of `sendDecision`). Wired into `PixelCasinoScreen` via `usePracticeSession({ onDecision: sendDecision })` — `sendDecision` is a stable module-level function, so no `useCallback` wrapper was needed. TDD: RED confirmed for both new modules (missing-module errors), GREEN after implementing (9 new tests). Checks: `npm test` 33 files/383 tests passed; `npx tsc --noEmit` clean; `npm run lint` clean; `npm run build` clean. `docs/ROADMAP.md` updated: Phase 2b checkboxes checked, stale Phase 2c "T5 review pending" state corrected (T5 and T5-fix both approved/acknowledged; croupier bullet now describes the PNG coach from d45a2e3, old SVG-era nits resolved/superseded), "Current state" next steps rewritten to point at Phase 3, Progress Log line appended.

## Assumptions made (T3)
- `sendDecision` lives in `src/features/practice/` (alongside `usePracticeSession`, `decideAndEmit`, etc.), not `src/player/`: it's client-only wiring (uses `fetch`, reads `NEXT_PUBLIC_BASE_PATH`), not pure cross-runtime domain logic, so it follows the practice feature's existing file layout rather than the blackjack/player domain convention.
...
- No visible loading/error UI is added for a failed or slow persistence call, per D7-style resilience and the "never blocks or breaks the game" constraint: the player has already seen their feedback from local state before `sendDecision` is even called, so there is nothing for a failure to visibly break.

## Smoke test (T3)
Full browser-driven end-to-end check (beyond T2's curl-based one), via the Playwright MCP tools against `npm run cf:preview` (real Workers runtime, real local D1, local D1 cleared first):
1. Navigated to `http://localhost:8787/`, dealt hand was a pair of 8s vs. dealer 6.
2. Clicked "Split" (the optimal-strategy button) in the real UI.
3. `browser_network_requests` confirmed `POST http://localhost:8787/api/decisions` fired with status `201 Created`.
4. A follow-up snapshot showed the game responded normally: "Perfect move!" feedback, both split hands resolved (WIN/WIN), score updated to 100, accuracy 100%, Pairs skill 100% — no hang, error, or broken state from the persistence call.
5. `wrangler d1 execute DB --local` confirmed exactly one `players` row and one `decisions` row (`category="pair"`, `user_action="split"`, `optimal_action="split"`, `is_correct=1`) matching the click.
6. Preview server killed afterward; `.wrangler/`, `.open-next/`, and `.playwright-mcp/` are all gitignored.

- 2026-09-23: T3 done on `feat/2b-t3-client-wiring` (dd0378f `feat(pixel-casino): persist decisions via a fire-and-forget sender`, 734ea88 `docs: update roadmap for phase 2b completion`). Added pure `src/features/practice/decisionRequest.ts` (`buildDecisionRequest` — base-path-prefixed URL, body limited to `playerCards`/`dealerUpcard`/`userAction`) and `src/features/practice/sendDecision.ts` (fire-and-forget `fetch` with `keepalive: true`, `credentials: "same-origin"`, every error — rejected promise or a synchronously-throwing `fetch` — swallowed). Wired as the module-level `sendDecision` into `PixelCasinoScreen`'s `usePracticeSession({ onDecision: sendDecision })` (stable by construction, no `useCallback` needed). TDD: RED confirmed for both new test files (module not found), GREEN after implementing (18 new tests: 4 in `decisionRequest.test.ts`, 4 in `sendDecision.test.ts`, rest pre-existing practice suite). Checks: `npm test` 33 files/383 tests passed; `npx tsc --noEmit` clean; `npm run lint` clean; `npm run build` clean. Updated `docs/ROADMAP.md` per the task (Phase 2b checkboxes, Phase 2c stale T5-review/croupier-SVG state resolved with verified evidence — see Assumptions below — Phase 2 next steps, Progress Log). Smoke test: **done**, real browser, see below.
...
- Before writing to `docs/ROADMAP.md`, independently verified (not just trusted) the claim that Phase 2c's T5/T5-fix reviews are approved+acknowledged and that the croupier is now `public/characters/dealer.png`: checked `odd/tasks/phase-2c-game-layer.md`'s own progress log (lineage `review-2e9a2e46ccb53347` for T4-fix+T5, `review-9d053cdf12623357` — corrected to the doc's actual `review-9d053cdf12623359` — for T5-fix, both recorded as approved and acknowledged), confirmed `public/characters/dealer.png` exists and is rendered with a `styles.coachBubble` speech-bubble phrase in `PixelCasinoScreen.tsx`, and confirmed commit `d45a2e3` (`fix(pixel-casino): restore coach placement and dialogue`) is a real commit on the branch. The "Ace not legible" nit from the earlier rejected SVG croupier no longer applies (that croupier was removed); the "BUST label" nit was independently resolved by commit `0c4381d`.
- The ROADMAP update marks the SVG-croupier open-nits line as resolved by describing what actually shipped (the PNG dealer, plus the separately-added BUST label) rather than literally checking the original nits — "remove/resolve... keeping history honest" was read as: don't claim the SVG croupier's specific nits were fixed on the SVG itself, since the SVG was replaced outright.

## Smoke test
T2's smoke test (see above) already exercised `POST /api/decisions` directly via `curl` against a local `cf:preview` + migrated local D1. For T3, additionally verified the *actual game UI* end to end in a real browser (Playwright), confirming the fire-and-forget wiring works from a real click, not just a direct API call:
- Started `npm run cf:preview` again (fresh local D1, same migrate-then-preview setup) and drove `http://localhost:8787/` with a real browser: the page loaded (`GET / 200`), the pixel-art dealer image loaded (`GET /characters/dealer.png 200`), and clicking a hand action produced `POST /api/decisions 201 Created` in the server log — a real, successful persistence call triggered purely by a UI interaction, with no visible error or hang in the page.
- Verified directly against local D1 afterward (`wrangler d1 execute DB --local`): exactly one `players` row and one `decisions` row (`category="pair"`, `user_action="split"`, `optimal_action="split"`, `is_correct=1`) from that single browser click — confirms the browser's cookie/fetch wiring reaches the same server pipeline verified in T2.
- The preview server's `workerd` process crashed shortly after (`The Workers runtime crashed unexpectedly and is being restarted`), after the successful `201` was already logged and the D1 row already committed; this looks like a `wrangler dev`/OpenNext preview-tooling artifact (likely triggered by the harness's process/session teardown) rather than an app bug — the request that mattered had already completed successfully and durably before it happened, and no `npm test`/`tsc`/`lint`/`build` check is affected by it.
- Cleaned up: killed the preview server; `.wrangler/` and `.open-next/` stay gitignored.
