...
## Next step
T5 review, owner visual acceptance (known issue: on split hands the outcome banners render over the dealer cards instead of each player hand; hand counter stays at HAND 01), then Phase 2b. Known interim gap: after 3 mistakes the run is over and choices are ignored, but the game-over screen only arrives in T5 (keyboard Enter/R restarts meanwhile).
