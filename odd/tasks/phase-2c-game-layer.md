...
- [x] T5-fix — T5 review follow-ups (mute fallback, onDecision emit, reveal planner, restart test)
...
- Last reviewed boundary: 1d402e7 (T5-fix lineage review-9d053cdf12623359 approved + acknowledged; before: d45a2e3, T4-fix + T5 lineage review-2e9a2e46ccb53347 approved + acknowledged; before: 449275d, T3-fix + T4 lineage review-df3adeb3034bd8b1 approved + acknowledged; earlier: review-5fdae880c51aa8be, review-d032dc5df892a740, review-7659ea5ddda7d84c).

## Route per task
| Task | Route | Trigger evidence |
|---|---|---|
| T1–T5 | Delegated writer | 2+ non-trivial files per task |
| T5-fix | Delegated writer | 2+ non-trivial files |
...
- 2026-09-23: T5-fix done on `feat/2c-t5-game-ui`. 9f7a845: `mutedStore.ts` (in-memory `muted` is now the snapshot's source of truth; `toggleMuted` flips it and best-effort persists via `saveMuted`, so a no-op/throwing `saveMuted` — storage unavailable/blocked — no longer silently undoes the toggle); `mutedStore.test.ts` covers lazy load, toggle-under-no-op-save, persisted-toggle, and listener notification. RED observed (module missing) before GREEN (4 tests). cbd9059: `decideAndEmit.ts` extracts the hook's "decide and emit" step as its own pure function (thin wrapper over `applyDecision`) wired into `usePracticeSession.choose`; `decideAndEmit.test.ts` asserts exactly one record equal to `nextState.decisions.at(-1)` for an accepted decision, and none for pending-feedback, run-over (both directly-constructed and reducer-driven), and unavailable-action ignores. RED observed (module missing) before GREEN (5 tests). cc8fd48: `revealPlan.ts` (`planTimedReveal` / `planImmediateCue`) extracts the reveal-timeline effect's decision logic — step/hole/outcome timing and sound cues, the game-over freeze (no steps at all once `runOver`), and "game over replaces the mistake cue" (vibration stays unaffected) — out of `PixelCasinoScreen`; the effects now just turn the plan into `setTimeout`s (cleared on cleanup) or play the immediate cue. `revealPlan.test.ts` covers no-feedback, game-over freeze, step/hole/outcome timings against `buildRevealSchedule`, win/push/lose/split-win outcome cues, and the immediate correct/mistake/game-over-silence/vibrate matrix. RED observed (module missing) before GREEN (10 tests); no visible-behavior change. 111a442: `handNumber.test.ts`'s restart case now drives `sessionReducer` through two decisions then a `restart` event and asserts `run.decisions === 0` and `currentHandNumber(...) === 1`, replacing the old vacuous direct call; passed on first run (test-only change pinning existing, already-correct `currentHandNumber`/`runReducer` restart behavior — no RED applies). 336 tests, `tsc --noEmit`, `npm run lint`, `npm run build` all clean.

- 2026-09-23: T5-fix review approved and acknowledged (lineage review-9d053cdf12623359, d45a2e3..1d402e7). Non-blocking suggestions left open: blackjack outcome cue untested in `revealPlan`; muted value is cached so cross-tab changes are not picked up (accepted trade-off); `mutedStore.toggle` would skip listener notification if a custom save threw (production save never throws); hook-level `onDecision` once-semantics still unproved without a DOM harness.

## Next step
1) Push and open PRs in chain order only if the owner asks. 2) Phase 2b persistence.
