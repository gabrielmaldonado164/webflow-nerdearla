...
Branch: `feat/4-ai-coach`, stacked on `feat/3-t4-skill-map-ui`. Work units: `2a0438e` (backend/API) and `71bff33` (game UI and documentation). Not pushed; commit `61e1259` deployed to Webflow Cloud main after owner approval.
...
Without either variable, `/api/coach/stream` returns 503; the game and EV evidence remain usable. `COMMAND_CODE_MODEL` is set in Webflow Cloud to `deepseek/deepseek-v4-flash` (verified against the provider's current public model catalog). Webflow Cloud lists `COMMAND_CODE_API_KEY` with `isSecret: true`; its value stayed masked, and successful live coach requests verified that it works.
...
- Owner-approved Webflow Cloud main deployment `6bf2ff66-7e4b-483c-b98c-754f8bd4af76` succeeded at `https://webflow-nerdearla.webflow.io/`. Public smoke: homepage 200; evidence 200 with optimal `hit` and three EV rows for hard 16 vs dealer 10; chat 200 with zero-decision stats for a new player; Why 200 with text/plain explanation and EV matching the engine.
- Browser UI smoke could not run because no browser connection was available. Move mobile/desktop visual QA to Phase 5. The live 429 edge was not exercised to avoid 18 extra paid requests; local SQLite threshold smoke and the unit test cover it.
