...
**Current state (2026-09-23):** T1-T3 (backend/logic) implemented and committed on a stacked branch chain (not pushed), on top of Phase 2b: `feat/2b-t3-client-wiring` → `feat/3-t1-stats` (c010c43) → `feat/3-t2-achievements` (913b609) → `feat/3-t3-adaptive-weighting` (7445ba7). Tracked in `odd/tasks/phase-3-stats-skill-map.md`. 454 tests, tsc, lint, build clean. T4 (Skill Map panel UI + "Practice weakness" toggle) is not started; out of this session's scope.
- [x] Stats API: totals, accuracy, current and best streak, strongest and weakest category (`GET /api/stats`, T1)
- [ ] Skill Map UI (per-category progress) — T4, not started
- [x] Adaptive scenario weighting (`weight = base + weaknessFactor`) logic and wiring (`weightsFromStats`, `usePracticeSession.setWeights`, T3); the "Practice weakness" button itself is UI — T4, not started
- [x] Achievements/badges derived from persisted decisions (`deriveAchievements`, included in `GET /api/stats`, T2); shown next to the Skill Map is UI — T4, not started
