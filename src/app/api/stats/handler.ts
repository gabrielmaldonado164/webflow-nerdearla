/**
 * Pure handler for `GET /api/stats` (Phase 3 T1). Everything the route
 * actually does — validating the cookie, fetching the player's decision
 * history, and computing stats — lives here behind injected dependencies
 * (`StatsHandlerDeps`), unit-testable with fakes instead of a real
 * Next.js `Request`/`cookies()`/D1 binding. `route.ts` is a thin adapter
 * wiring these deps to the real runtime.
 *
 * Never mints or sets a cookie (unlike `POST /api/decisions`): an
 * absent or invalid cookie means there is no known player, so the
 * response is always empty stats, and the repository is never queried.
 *
...
export interface StatsHandlerResult {
  status: number;
  body: { stats: PlayerStats } | { error: string };
}
...
export async function handleStatsRequest(deps: StatsHandlerDeps): Promise<StatsHandlerResult> {
  ...
  if (!isValidPlayerId(cookieValue)) {
    return { status: 200, body: { stats: computePlayerStats([]) } };
  }

  try {
    const decisions = await deps.repo.listPlayerDecisions(cookieValue);
    return { status: 200, body: { stats: computePlayerStats(decisions) } };
  } catch (error) {
